...
import tempfile
import shutil
from analysis_utils import load_swarm_data, apply_swarm_offsets
...
class TestSwarmIntegration(unittest.TestCase):
    def setUp(self):
        ...
        # Mock assumptions
        self.assumptions = {"defaults": {"city": "Reykjavik, IS", "lat": 64.1265, "lng": -21.8174, "timezone": "Atlantic/Reykjavik"}}
    ...
    def test_apply_swarm_offsets(self):
        ...
        tracks = [
            ...
            {"timestamp": int(pd.to_datetime("2026-01-01 13:00:00", utc=True).timestamp()), "date_text": "2026-01-01 13:00:00"},
            # 1 hour after second checkin (2026-01-02 12:00 UTC)
            {"timestamp": int(pd.to_datetime("2026-01-02 13:00:00", utc=True).timestamp()), "date_text": "2026-01-02 13:00:00"}
        ]
