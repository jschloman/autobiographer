...
def card(
    ...
    """Draw a rounded card background on the figure."""
...
def label(
    ...
    fig.text(x, y, text, color=color, fontsize=size, fontweight=weight, ha=ha, va=va)
...
# ══════════════════════════════════════════════════════════════════════════════
SB_W = 0.13
sidebar_bg = plt.Rectangle(
    (0, 0), SB_W, 1, facecolor=SIDEBAR, transform=fig.transFigure, clip_on=False
)
fig.add_artist(sidebar_bg)
sidebar_border = plt.Rectangle(
    (SB_W - 0.001, 0), 0.001, 1, facecolor=BORDER, transform=fig.transFigure, clip_on=False
)
...
fig.text(
    0.065, 0.905, "personal data explorer", color=TEXT_MUTED, fontsize=6.5, ha="center", va="center"
)
...
for i, ((_icon, col, active), nav_label) in enumerate(zip(nav_items, nav_labels)):
    y = 0.845 - i * 0.065
    if active:
    ...
    dot = plt.Circle((0.038, y), 0.007, color=col, transform=fig.transFigure, clip_on=False)
    fig.add_artist(dot)
    fig.text(
        0.052,
        y,
        nav_label,
        color=col if active else TEXT_DIM,
        fontsize=7.5,
        fontweight="semibold" if active else "normal",
        va="center",
    )

# Config fields
fig.text(
    0.016, 0.38, "DATA SOURCES", color=TEXT_MUTED, fontsize=5.5, fontweight="bold", va="center"
)
...
fig.add_artist(btn)
fig.text(
    0.065, 0.189, "Load Data", color=TEXT, fontsize=7.5, fontweight="bold", ha="center", va="center"
)
...
# ── Page title ─────────────────────────────────────────────────────────────
fig.text(MX, 0.955, "Overview", color=TEXT, fontsize=16, fontweight="bold", va="center")
fig.text(
    MX,
    0.928,
    "Your complete personal data snapshot  ·  2024",
    color=TEXT_DIM,
    fontsize=8,
    va="center",
)
...
ax_hero.imshow(
    grad_data, aspect="auto", cmap=hero_cmap, extent=(0, 1, 0, 1), origin="lower", zorder=0
)
...
fig.text(HX + 0.018, HY + HH * 0.22, "scrobbles in 2024", color=TEXT_DIM, fontsize=8, va="center")
...
for i, (val, delta, lbl, accent) in enumerate(kpi_data):
    kx = MX + i * (KW + GAP)
    card(kx, KY, KW, KH, accent_left=accent)
    fig.text(
        kx + 0.016, KY + KH * 0.72, val, color=TEXT, fontsize=14, fontweight="bold", va="center"
    )
...
for _i, (y, c) in enumerate(zip(normalized, genre_colors)):
...
for i, (g, c) in enumerate(zip(genres, genre_colors)):
    dot = plt.Circle(
        (legend_x + i * 0.072 + 0.005, R2Y + 0.022),
        0.003,
        color=c,
        transform=fig.transFigure,
        clip_on=False,
    ...
    fig.add_artist(dot)
    fig.text(
        legend_x + i * 0.072 + 0.011, R2Y + 0.022, g, color=TEXT_DIM, fontsize=5.5, va="center"
    )

# ── Top Artists horizontal bar ──────────────────────────────────────────────
BA_X = MX + SG_W + GAP / 2
BA_W = MW - SG_W - GAP / 2
card(BA_X, R2Y, BA_W, R2H)
...
bars = ax_bar.barh(range(len(artists)), counts, color=bar_colors, height=0.65, zorder=2)
...
for i, (_bar, count) in enumerate(zip(bars, counts)):
    ax_bar.text(count + max_c * 0.02, i, f"{count:,}", va="center", color=TEXT_DIM, fontsize=5.5)
...
for cx, cy, radius, color in cluster_centers:
    for r in np.linspace(radius, 0, 12):
        ...
        circle = plt.Circle(
            (cx, cy), r, color=color, alpha=alpha, transform=ax_map.transData, zorder=2
        )
...
ax_map.scatter(
    dot_x[:n_dots], dot_y[:n_dots], s=4, c=dot_colors[:n_dots], alpha=0.7, zorder=3, linewidths=0
)
...
map_caption = "pydeck  ·  HeatmapLayer + ScatterplotLayer  ·  CARTO dark-matter"
fig.text(MX + 0.008, R3Y + 0.024, map_caption, color=TEXT_MUTED, fontsize=5.5, va="center")

# ── Calendar Heatmap ────────────────────────────────────────────────────────
CAL_X = MX + MAP_W + GAP / 2
CAL_W = MW - MAP_W - GAP / 2
card(CAL_X, R3Y, CAL_W, R3H)
...
ax_cal.imshow(
    grid, aspect="auto", cmap=cmap_cal, vmin=0, vmax=35, origin="upper", interpolation="nearest"
)
ax_cal.set_xticks(np.linspace(0, cols - 1, 12))
ax_cal.set_xticklabels(
    ["J", "F", "M", "A", "M", "J", "J", "A", "S", "O", "N", "D"], color=TEXT_DIM, fontsize=5.5
)
...
fig.text(
    0.5,
    0.015,
    "autobiographer  ·  design mockup  ·  deep space indigo palette",
    color=TEXT_MUTED,
    fontsize=6,
    ha="center",
    va="center",
