...
class Autobiographer:
    ...
    def fetch_recent_tracks(self, limit: int = 200, pages: Optional[int] = None, from_ts: Optional[int] = None, to_ts: Optional[int] = None) -> List[Dict]:
    ...
    def save_tracks_to_csv(self, tracks: List[Dict], filename: Optional[str] = None):
...
def main():
    ...
    parser.add_argument("--user", help="Last.fm username (defaults to AUTOBIO_LASTFM_USERNAME env var)")
    ...
    if not all([api_key, api_secret, username]):
        print("Error: AUTOBIO_LASTFM_API_KEY, AUTOBIO_LASTFM_API_SECRET, and AUTOBIO_LASTFM_USERNAME must be set.")
        return
